  ...
  const { data: items, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select('id, title, category, description, instructor, location, schedule, duration, is_free, price, vacancies, cover_url')
        .eq('is_active', true)
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-primary)' }}>
      <div style={{ background: 'var(--bg-inst-header)', borderBottom: '3px solid var(--accent)' }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold" style={{ color: 'var(--text-inst-title)' }}>Oficinas Culturais</h1>
          </div>
          <p style={{ color: 'var(--text-inst-subtitle)' }}>Cursos, workshops e capacitações em arte e cultura</p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex flex-col sm:flex-row gap-4 mb-8 items-start sm:items-center justify-between">
          ...
          <a href="/oficinas/matricula" className="btn btn-primary shrink-0 gap-2">
            <ClipboardList size={16} /> Ficha de Matrícula
          </a>
        </div>
            ...
            {filtered.map((item: any) => (
              <div key={item.id} className="group rounded-2xl overflow-hidden border transition-all hover:shadow-lg hover:-translate-y-1" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                ...
                <div className="p-5">
                  ...
                  {item.instructor && <p className="text-sm mb-1" style={{ color: 'var(--text-secondary)' }}>{item.instructor}</p>}
                  {item.description && <p className="text-sm line-clamp-2 mb-3" style={{ color: 'var(--text-secondary)' }}>{item.description}</p>}
                  <a
                    href={`/oficinas/${item.id}/matricula`}
                    className="btn btn-primary w-full text-sm py-2 gap-1.5 justify-center"
                    onClick={e => e.stopPropagation()}
                  >
                    <ClipboardList size={14} /> Ficha de Matrícula
                  </a>
                </div>
              </div>
            ))}
