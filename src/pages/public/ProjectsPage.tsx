  ...
  const { data: items, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select('id, title, status, description, coordinator, contact, start_date, cover_url')
        .eq('is_active', true)
      ...
      <div style={{ background: 'var(--bg-inst-header)', borderBottom: '3px solid var(--accent)' }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold" style={{ color: 'var(--text-inst-title)' }}>Projetos Culturais</h1>
          </div>
          <p style={{ color: 'var(--text-inst-subtitle)' }}>Iniciativas e programas culturais do município</p>
        </div>
      </div>
            ...
            {filtered.map((item: any) => (
              <div key={item.id} className="group rounded-2xl overflow-hidden border transition-all hover:shadow-lg hover:-translate-y-1" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                ...
                <div className="p-5">
                  ...
                  {item.coordinator && <p className="text-sm mb-2" style={{ color: 'var(--text-secondary)' }}>{item.coordinator}</p>}
