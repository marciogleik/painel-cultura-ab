      ...
      <div style={{ background: 'var(--bg-inst-header)', borderBottom: '3px solid var(--accent)' }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold" style={{ color: 'var(--text-inst-title)' }}>Espaços Culturais</h1>
          </div>
          <p style={{ color: 'var(--text-inst-subtitle)' }}>Teatros, museus, centros culturais e espaços de arte de Água Boa</p>
        </div>
      </div>
              ...
              <div key={item.id} className="group rounded-2xl overflow-hidden border transition-all hover:shadow-lg hover:-translate-y-1" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                <div className="aspect-video overflow-hidden" style={{ background: 'var(--bg-secondary)' }}>
                  {item.cover_url ?? item.photo_url ? (
                    <img src={item.cover_url ?? item.photo_url} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                  ) : (
                ...
                <div className="p-5">
                  <h3 className="font-bold text-base leading-snug mb-1" style={{ color: 'var(--text-primary)' }}>{item.name}</h3>
                  {item.address && <p className="text-sm mb-2" style={{ color: 'var(--text-secondary)' }}>{item.address}</p>}
